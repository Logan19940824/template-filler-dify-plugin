...
The plugin provides eleven tools. Call `template_filler_usage_guide` first for the recommended workflow and JSON schemas. The template tools use a public `http` or `https` URL. `inspect_word_document` and `insert_word_placeholders` expose DOCX body/table locations and apply validated placeholder operations. The corresponding Excel tools inspect and edit XLSX workbooks. Placeholders use the `{{name}}` syntax. The parse tools return a `values` object that can be completed and passed to the corresponding fill tool as JSON, for example `{"customer_name":"Alice"}`. Generated files are uploaded to Dify and returned as download links.
